...
export class Program {
  ...
  _attributes: Object;
  _uniforms: Map<string, any>;
  _program: WebGLProgram;
  _vertexSource: string;
  _fragmentSource: string;
  _transformFeedback: any;
  _error: any;
   ...
   */
  setSource(vertexSource, fragmentSource) {
    let gl = this._gl,
      vertexShader,
      fragmentShader,
      program,
    ...
    function loadShader(type, source) {
      ...
      if (
        ...
        this._error = `Create ${errorType} shader ${gl.getShaderInfoLog(
          shader
        )}\n${source}`;
    ...
    if (this._transformFeedback) {
      ...
      //   gl[this._transformFeedback.bufferMode || 'INTERLEAVED_ATTRIBS']
      // );
    }
...
export function createProgram(vertex, fragment, transformFeedback) {
  return new Program({
